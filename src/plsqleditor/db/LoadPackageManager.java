...
public class LoadPackageManager {
    ...
    /**
     * This method executes a piece of code (primarily a package body or header, or
     * similar piece of ddl (rather than dml) into the database. This may be
     ...
     *         loaded code.
     */
    public synchronized SQLErrorDetail[] execute(IFile file, IProject project, String schema, String packageName,
	...
	if (useLocalClient && (executable != null && executable.length() > 0)) {
	    ...
	    SqlPlusProcessExecutor sqlplus = new SqlPlusProcessExecutor(executable, schema, passwd, sid);
	    try {
		...
		for (Iterator<Segment> it = sortedSegments.iterator(); it.hasNext();) {
			...
			if (segment.getType() == SegmentType.Package_Body) {
			    ...
			    details = loadSinglePieceOfCode(file, project, schema, packageName, packageHeader,
				    PackageType.Package);
			    SQLErrorDetail[] details2 = loadSinglePieceOfCode(file, project, schema, packageName,
				...
				newDetails[j] = new SQLErrorDetail(oldDetails2.getRow() + packageBodyStartingLine,
					oldDetails2.getColumn(), oldDetails2.getText());
			    }
    ...
    /**
     * This method executes a single piece of code in the database. If the file does
     * not have a specific connection open against it, this function will then
     * commits the loaded code (via auto commit on close functionality) before
     * closing the connection, and freeing it back to the connection pool. This is
     * always executed by jdbc. If the file has a specific connection against it,
     * that connection is chosen and no commit is executed.
     *
     * @param file        The file that is being executed on. This is required in
     *                    case there is a specific connection set against the file.
     *
     * @param schemaName  The schema against which to retrieve a connection.
     *
     * @param packageName The name of the package (piece of code) being executed.
     *
     ...
     *         loaded code.
     */
    private synchronized SQLErrorDetail[] loadSinglePieceOfCode(IFile file, IProject project, String schemaName,
	    String packageName, String toLoad, PackageType type) {
	Connection c = null;
	...
	if (myFileToSpecificConnectionMap.containsKey(fullFilename)) {
		...
		e.printStackTrace();
		final String msg = "Failed to retrieve connection for file " + fullFilename + " with type " + type
			+ " : " + e.getMessage();
		return new SQLErrorDetail[] { new SQLErrorDetail(0, 0, msg) };
	    }
	} else {
		...
		final String msg = "Failed to retrieve connection for schema " + schemaName + " package " + packageName
			+ " and type " + type + " : " + e.getMessage();
		return new SQLErrorDetail[] { new SQLErrorDetail(0, 0, msg) };
	    } finally {
		if (c != null) {
     ...
     *
     * @param type        The type of thing being loaded. (package/package body
     *                    etc).
     *
     * @return The list of errors from the compile, or null if there were none.
     */
    private SQLErrorDetail[] loadCode(Connection c, String packageName, String toLoad, PackageType type) {
     ...
     * unless there is a specific connection set for the given file. In this case
     * the specific connection is used, and the connection remains open. The
     * connection is left unfreed until the caller closes the returned result set
     * wrapper. However, a subsequent call to this method will close any previous
     * result set for the given <code>schemaName</code> (or file). If there is no
     ...
     *                   specific connection against it, that connection will be
     *                   used instead of the default schema connection, and it will
     *                   not be autoclosed. Instead, the connection will remain as
     *                   an open session.
     ...
     *
     * @return The results set (wrapped) resulting from the execution of the code
     *         <code>toLoad</code>. If there was no result set, null is returned.
     *
     * @throws SQLException when the executed code fails for some reason.
     */
    public synchronized ResultSetWrapper loadCode(IFile file, IProject project, String schemaName, String toLoad)
	    throws SQLException {
	String fullFilename = file.getFullPath().toString();
	if (myFileToSpecificConnectionMap.containsKey(fullFilename)) {
	    ConnectionHolder ch = myFileToSpecificConnectionMap.get(fullFilename);
     ...
     * @param end
     * @param freq
     * @return the time elapsed between the <code>start</code> and <code>end</code>
     *         over the <code>freq</code>.
     */
    ...
    /**
     * This method returns the result set wrapper that is currently stored against
     * the supplied <code>schemaName</code>. This represents the output of the last
     * piece of code executed in the database against that schema. This may be null
     * if there have been no pieces of code executed against this schema, or the
     * last execution had no result.
     *
     * @param schemaName The name of the schema whose last result set is sought.
     *
     * @return the result set wrapper that is currently stored against the supplied
    ...
    /**
     * This method gets the error status of the statement <code>s</code>. If there
     * have been warnings, the error details will be initialised with the new
     * errors, and the first warning message will be returned to indicate something
     * is wrong.
     *
     * @param s           The statement whose warning status is being checked.
     *
     * @param packageName The name of the package/package body that was just created
     *                    in the statement <code>s</code>.
     *
     ...
     *         warnings.
     */
    private String getErrorStatus(Connection c, Statement s, String schemaName, String packageName,
	...
	try {
		...
		if (packageType == PackageType.Sql) {
		    ...
		    SQLErrorDetail[] details = getErrorDetails(c, schemaName, packageName, packageType);
    ...
    public SQLErrorDetail[] getErrorDetails(IProject project, String schemaName, String packageName,
    ...
    /**
     * This method gets all the error details currently in the db for the schema
     * that backs the supplied connection <code>c</code>.
     *
	...
	String sql = "select line, position, text, name from user_errors" + " order by sequence";
	...
	try {
	    ...
	    while (rs.next()) {
		detail = new SQLErrorDetail(rs.getInt(1), rs.getInt(2), rs.getString(4) + ": " + rs.getString(3));
		v.add(detail);
	    }
     ...
     * create/replace the procedure/function of type <code>procType</code> and name
     * <code>procName</code>.
     *
     * @param c           The connection to use to create the error request
     *                    statement.
     *
     * @param packageName The name of the package or package body that was compiled,
     *                    but caused warnings.
     *
