...
public class MultiOptionMultiLineRule implements IPredicateRule
{
	...
	public MultiOptionMultiLineRule(String startSequence,
			String[] endSequences, IToken token, char escapeCharacter,
			boolean breaksOnEOF)
	{
		assert startSequence != null && startSequence.length() > 0;
		assert token != null;
		...
		while ((c = scanner.read()) != ICharacterScanner.EOF)
		{
					...
					for (int i = 0; i < fSortedLineDelimiters.length; i++)
					{
						if (c == fSortedLineDelimiters[i][0]
								&& sequenceDetected(scanner,
										fSortedLineDelimiters[i], true)) break;
					}
				}
				else scanner.read();
			...
			else if (charStartsAnEndSequence(c))
			{
				...
				for (int i = 0; i < myEndSequences.length; i++)
				{
					...
					{
						if (sequenceDetected(scanner, myEndSequences[i], true))
							return true;
						// scanner.unread();
					}
				}
