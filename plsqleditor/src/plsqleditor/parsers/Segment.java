...
public class Segment implements Comparable, Cloneable, IMultilineElement
{
	...
	protected List<Segment> myFieldList = new ArrayList<Segment>();
	...
	/**
	 * This contains information that is used by the
	 * {@link plsqleditor.editors.PlSqlTextHover} class.
	 */
	...
	public Segment(String name, Position position)
	{
		...
		this.myName = name.replaceFirst("(" + FUNCTION + "|" + PROCEDURE
				+ ") +(\\w+)", "$2");
		this.myName = this.myName.replaceFirst("(" + TYPE + ")? +(\\w+).*",
				"$2");
		this.myName = this.myName.replaceFirst("(" + PRAGMA + ")? +(\\w+).*",
				"$2");
	...
	public String getPresentationName(boolean isShowingParameterList,
			boolean isShowingReturnType, boolean overrideParameterSettings)
	{
		return myName.toLowerCase()
				+ (isShowingParameterList ? getParameterListAsString(overrideParameterSettings)
						: "")
				+ (isShowingReturnType && isValidReturnTypeType() ? " : "
						+ myReturnType : "");
	}
	...
	public String toString()
	{
		return getPresentationName(isShowingParameterList(),
				isShowingReturnType(), false);
	}
	...
	public List<Segment> getParameterList()
	{
		...
		for (Iterator<Parameter> it = myParameterList.iterator(); it.hasNext();)
		{
			...
			{
				s.addLine(p.myExtraDetails, new Position(pos.offset
						+ p.myParameter.length() + p.myParamType.length() + 2,
						p.myExtraDetails.length()));
			}
	...
	{
		return prefs().getBoolean(
				PreferenceConstants.P_IS_SHOWING_PARAMETER_LIST);
	}
	 ...
	 * @param paramType
	 */
	public void addParameter(String paramName, String paramInOut,
			String paramType, String extraDetails, int offset)
	{
	 ...
	 * 
	 * @param field
	 *            The segment field to add to the list of fields.
	 */
	...
	public int hashCode()
	{
		return new HashCodeBuilder(23, 397).append(this.myType)
				.append(this.myName).append(this.myParameterList)
				.append(this.myPosition.getOffset())
				.append(this.myPosition.getLength())
				.append(this.myLastPosition.getOffset())
				.append(this.myLastPosition.getLength())
				.append(this.myReturnType).append(this.myDocumentation)
				.append(this.myIsPublic).toHashCode();
	}
	...
	public int compareTo(Object o)
	{
		...
		return new CompareToBuilder()
				.append(this.myType, rhs.myType)
				.append(this.myName, rhs.myName)
				.append(this.myParameterList.toArray(new Parameter[myParameterList
						.size()]),
						rhs.myParameterList
	 ...
	 * 
	 * @param isPublicSynonym
	 *            Whether this segment is a public synonym or not.
	 */
	...
	public String format()
	{
		...
		{
			sb.append(type).append(" ").append(getName()).append(" IS ")
					.append(getReturnType());
		}
		else if (type == SegmentType.Procedure || type == SegmentType.Function)
		{
			...
			{
				sb.append("    ").append("RETURN ").append(returnType)
						.append("\n");
			}
			sb.append("    ").append("IS\n");
		...
		parseLines(sb, "    ");
		if (type == SegmentType.Field || type == SegmentType.Constant
				|| type == SegmentType.Type || type == SegmentType.SubType)
		{
			sb.append(";");
		}
		sb.append("\n");
	...
	public String getImageKey()
	{
		return getType().toString()
				+ (isPublic() ? Segment.PUBLIC : Segment.PRIVATE);
	}
	 ...
	 * Essentially it is determining whether this source comes from a package
	 * body or header.
	 * 
	 ...
	 *         <li>SegmentType.Code - if the parent type could not be
	 *         determined.</li>
	 *         </ul>
	 */
	public SegmentType deriveSourceSegmentType()
	{
		if (myType == SegmentType.Package || myType == SegmentType.Package_Body)
		{
			return myType;
		}
	...
	{
		return myLastPosition.offset + myLastPosition.length
				- myPosition.offset;
	}
