...
public class PlSqlPackage
{
     ...
     * {@link ParseType}.
     */
    private TreeMap     mySources       = new TreeMap();

    /**
     * This field contains the modification timestamps of the file from which
     * the last segment modification to this object came.
     */
    private Map         myLatestChanges = new HashMap();

    public PlSqlPackage(PlSqlSchema owner, String name, Source source, ParseType parseType)
    {
        mySchema = owner;
        myName = name;
    ...
    public boolean equals(Object object)
    {
        ...
        return new EqualsBuilder().append(this.myName, rhs.myName).append(this.mySegments,
                                                                          rhs.mySegments)
                .append(this.mySources, rhs.mySources).isEquals();
    }
    ...
    public int hashCode()
    {
        return new HashCodeBuilder(23, 397).append(this.myName).append(this.mySegments)
                .append(this.mySources).toHashCode();
    }
    ...
    public int compareTo(Object o)
    {
        ...
        return new CompareToBuilder().append(this.myName, rhs.myName).append(this.mySegments,
                                                                             rhs.mySegments)
                .append(this.mySources, rhs.mySources).toComparison();
    }
     ...
     */
    public void setSegments(List segments, long timestamp)
    {
        mySegments = segments;
     ...
     * 
     * @param segment The segment whose derived segment type will be used to
     *            determine the type of plsql code whose segments are being
     *            updated.
     * 
     * @param timestamp The modification time of the supplied
     *            <code>segment</code>.
     */
    private void setTimestamp(Segment segment, long timestamp)
    {
        SegmentType segmentType = segment.deriveSourceSegmentType();
        ParseType parseType = segmentType == SegmentType.Package
                ? ParseType.Package
                : segmentType == SegmentType.Package_Body ? ParseType.Package_Body : null;
        if (parseType != null)
        {
    ...
    public Source getPrimarySource()
    {
        ...
        for (Iterator it = mySources.keySet().iterator(); it.hasNext();)
        {
            ParseType type = (ParseType) it.next();
            Source source = (Source) mySources.get(type);
            if (toReturn == null)
            {
                toReturn = source;
                toReturnType = type;
     ...
     * 
     * @param segment The segment whose membership in the list of
     *            <code>segments</code> is being checked.
     * 
     * @param segments The list of segments which may contain the supplied
     *            <code>segment</code>.
     * 
     * @return <code>true</code> if the <code>segment</code> is contained in
     *         the list of <code>segments</code>.
     */
    private boolean contains(Segment segment, List segments)
    {
        String signature = segment.getSignature();
        for (Iterator it = segments.iterator(); it.hasNext();)
        {
            Segment toCheck = (Segment) it.next();
     ...
     * <code>toSubstituteInto</code> with the segment
     * <code>toSubstitute</code>. If there is no segment of the same name,
     * nothing is done.
     * 
     * @param toSubstitute The segment that will replace the previously stored
     ...
     * 
     * @param toSubstituteInto The list to substitute the segment into.
     * 
     * @return <code>true</code> if a segment was actually replaced, and
     *         <code>false</code> otherwise.
     */
    private boolean replaceSameSegment(Segment toSubstitute, List toSubstituteInto)
    {
     ...
     * 
     * @param segments The list of segments to reconcile with this package.
     * 
     * @param parseType The type of the file from which these segments came.
     * 
     * @param timestamp The timestamp of the file from which the
     *            <code>segments</code> were obtained.
     */
    public void reconcile(List segments, ParseType parseType, long timestamp)
    {
        if (mySegments == null)
        {
            mySegments = segments;
        ...
        {
            Segment thisPackageSegment = AbstractPlSqlParser.getPackageSegment(mySegments, myName);
            List thisPackagesSegments = mySegments;
            if (thisPackageSegment != null)
            {
                thisPackagesSegments = thisPackageSegment.getContainedSegments();
            }
            Segment packageSegment = AbstractPlSqlParser.getPackageSegment(segments, myName);

            List segmentsToProcess = segments;
            ...
            {
                for (Iterator it = segmentsToProcess.iterator(); it.hasNext();)
                {
     ...
     * 
     * @param parseType The type of source whose segments are being cleared (for
     *            update).
     * 
     * @param segmentsToClear The list to clear of segments of the type
     *            associated with the supplied <code>parseType</code>.
     */
    private void clearSegments(ParseType parseType, List segmentsToClear)
    {
        SegmentType toCompare = parseType == ParseType.Package
                ? SegmentType.Package
                : parseType == ParseType.Package_Body ? SegmentType.Package_Body : SegmentType.Code;
     ...
     * 
     * @param parseType The type of information source being checked.
     * 
